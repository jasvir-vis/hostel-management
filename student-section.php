...
$q = "SELECT *  From `student_profile` where email = '{$email}' and password = '{$pass}'";
...
if($num >0 ){
  while($row = mysqli_fetch_assoc($result)){
    if($email = $row['email'] && $pass = $row['password']){
      ...
      $_SESSION['login'] = true;
      $login = true;
      echo '<script> window.location.href = "student-data.php" </script>';
    }
    else{
      $login = false;
          ...
          <div class="row">
            <div class="col-12">
              ...
              <div class="d-flex gap-2 gap-md-4 flex-column flex-md-row justify-content-md-end">
                <a href="forgot.php" class="link-primary text-decoration-none">Forgot password</a>
              </div>
            </div>
          </div>
        </div>
