<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="bootstrap-5.3.2-dist/css/bootstrap.min.css">
    <link rel="stylesheet" href="style.css">
    <title>Fee Structure</title>
</head>
<style>
td {
    border: 1px solid black;
}

button a{
    text-decoration: none;
    color: white;
}
</style>

<body>
    <!-- navbar -->
    <nav class="navbar d-md-flex justify-content-between align-items-center bg-dark" id="top">
        <div class="logo d-md-flex align-item-center my-md-0 my-3 mx-md-0 mx-auto">
            <img src="hostel-image/logo.jpg" alt="">
            <h5><b>|~|OSTEL</b></h5>
        </div>
        <div class="hi w-75 d-lg-flex justify-content-around" id="jas">
            <div class="link">
                <ul class="nav-links mt-lg-0 mt-md-3 list-unstyled d-md-flex">
                    <li class="mx-lg-3 mx-1"><a href="index.php">Home</a></li>
                    <li class="mx-lg-3 mx-1"><a href="mess.php">Mess Schedule</a></li>
                    <li class="mx-lg-3 mx-1"><a href="Fees structure.php">Fees Structure</a></li>
                    <li class="mx-lg-3 mx-1"><a href="student-section.php">Student section</a></li>
                    <li class="mx-lg-3 mx-1"><a href="gallery.php">Gallery</a></li>
                    <li class="mx-lg-3 mx-1"><a href="about.php">About</a></li>
                </ul>
            </div>
            <div class="cta-btn my-md-0 my-3 mx-md-0 mx-auto">
                <a href="register.php">Apply Now</a>
            </div>
        </div>
    </nav>
    <div class="burger d-lg-none">
        <div class="line"></div>
        <div class="line"></div>
        <div class="line"></div>
    </div>
   
    <section id="print-content">
    <div class="container my-5" >
        <div class="container mt-5">
            <h1 class="text-center mb-5">Hostel Fees Structure</h1>
            <h4 class="text-secondary border-bottom p-3">~ NON AC ROOM : This is fees according to non ac room </h4>
            <p class="p-2 text-muted"><b>NOTE : Addmission and hostel deposit fees only conduct by addmission time</b>
            </p>

            <div class="container py-4">
    <button class="btn btn-outline-danger text-black" onclick="printDiv('print-content')">PRINT <i class="fa fa-x fa-print"></i></button>
  </div>
            <div class="row">
                <!-- Single Seater Room -->
                <div class="col-md-4 mb-4">
                    <div class="border">
                        <div class="d-flex flex-column justify-content-center align-item-center"
                            style="background: rgba(0,0,0,0.5) url(hostel-image/single.jpg); background-blend-mode: darken;">
                            <h4 class="p-3 text-center text-white">Single Seater Room</h4>
                            <button class="btn btn-primary btn-sm"><a href="hostel management System Project/hostel-fee/index.php">Apply now</a></button>
                        </div>
                        <table>
                            <thead>
                                <tr class="p-3 bg-dark text-white">
                                    <th class="p-3">Sr no.</th>
                                    <th class="p-3">Particular/month</th>
                                    <th class="p-3">Fees structure</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">1</td>
                                    <td class="p-3">Hostel addmision fees(one time)</td>
                                    <td class="p-3"> ₹500</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">2</td>
                                    <td class="p-3">Room rent</td>
                                    <td class="p-3"> ₹1000</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">3</td>
                                    <td class="p-3">Water & Electricity bill</td>
                                    <td class="p-3"> ₹2000</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">4</td>
                                    <td class="p-3">Medical checkup</td>
                                    <td class="p-3"> ₹300</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">5</td>
                                    <td class="p-3">Maintainance charges</td>
                                    <td class="p-3"> ₹200</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">6</td>
                                    <td class="p-3">Mess fees</td>
                                    <td class="p-3"> ₹2500</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">7</td>
                                    <td class="p-3">Hostel deposit(one time-Non-refundable)</td>
                                    <td class="p-3"> ₹500</td>
                                </tr>
                                <tr class="p-3 text-primary">
                                    <td class="p-3" colspan="2">6 Month Total</td>
                                    <td class="p-3"> ₹37,000</td>
                                </tr>
                                <tr class="p-3 text-primary">
                                    <td class="p-3" colspan="2">1 year Total</td>
                                    <td class="p-3"> ₹73,000</td>
                                </tr>
                            </tbody>

                        </table>
                    </div>
                </div>

                <!-- Double Seater Room -->
                <div class="col-md-4 mb-4">
                    <div class="border">
                        <div class="d-flex flex-column justify-content-center align-item-center"
                            style="background: rgba(0,0,0,0.5) url(hostel-image/single.jpg); background-blend-mode: darken;">
                            <h4 class="p-3 text-center text-white">Two Seater Room</h4>
                            <button class="btn btn-info btn-sm"><a href="hostel management System Project/hostel-fee/index.php">Apply now</a></button>
                        </div>
                        <table>
                            <thead>
                                <tr class="p-3 bg-dark text-white">
                                    <th class="p-3">Sr no.</th>
                                    <th class="p-3">Particular/month</th>
                                    <th class="p-3">Fees structure</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">1</td>
                                    <td class="p-3">Hostel addmision fees(one time)</td>
                                    <td class="p-3"> ₹500</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">2</td>
                                    <td class="p-3">Room rent</td>
                                    <td class="p-3"> ₹1000</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">3</td>
                                    <td class="p-3">Water & Electricity bill</td>
                                    <td class="p-3"> ₹1000</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">4</td>
                                    <td class="p-3">Medical checkup</td>
                                    <td class="p-3"> ₹300</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">5</td>
                                    <td class="p-3">Maintainance charges</td>
                                    <td class="p-3"> ₹200</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">6</td>
                                    <td class="p-3">Mess fees</td>
                                    <td class="p-3"> ₹2500</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">7</td>
                                    <td class="p-3">Hostel deposit(one time-Non-refundable)</td>
                                    <td class="p-3"> ₹500</td>
                                </tr>
                                <tr class="p-3 text-primary">
                                    <td class="p-3" colspan="2">6 Month Total</td>
                                    <td class="p-3"> ₹31,000</td>
                                </tr>
                                <tr class="p-3 text-primary">
                                    <td class="p-3" colspan="2">1 year Total</td>
                                    <td class="p-3"> ₹61,000</td>
                                </tr>
                            </tbody>

                        </table>
                    </div>
                </div>
                <!-- Four Seater Room -->
                <div class="col-md-4 mb-4">
                    <div class="border">
                        <div class="d-flex flex-column justify-content-center align-item-center"
                            style="background: rgba(0,0,0,0.5) url(hostel-image/single.jpg); background-blend-mode: darken;">
                            <h4 class="p-3 text-center text-white">Four seater Room</h4>
                            <button class="btn btn-dark btn-sm"><a href="hostel management System Project/hostel-fee/index.php">Apply now</a></button>
                        </div>
                        <table>
                            <thead>
                                <tr class="p-3 bg-dark text-white">
                                    <th class="p-3">Sr no.</th>
                                    <th class="p-3">Particular/month</th>
                                    <th class="p-3">Fees structure</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">1</td>
                                    <td class="p-3">Hostel addmision fees(one time)</td>
                                    <td class="p-3"> ₹500</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">2</td>
                                    <td class="p-3">Room rent</td>
                                    <td class="p-3"> ₹1000</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">3</td>
                                    <td class="p-3">Water & Electricity bill</td>
                                    <td class="p-3"> ₹400</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">4</td>
                                    <td class="p-3">Medical checkup</td>
                                    <td class="p-3"> ₹200</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">5</td>
                                    <td class="p-3">Maintainance charges</td>
                                    <td class="p-3"> ₹200</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">6</td>
                                    <td class="p-3">Mess fees</td>
                                    <td class="p-3"> ₹2200</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">7</td>
                                    <td class="p-3">Hostel deposit(one time-Non-refundable)</td>
                                    <td class="p-3"> ₹500</td>
                                </tr>
                                <tr class="p-3 text-primary">
                                    <td class="p-3" colspan="2">6 Month Total</td>
                                    <td class="p-3"> ₹25,000</td>
                                </tr>
                                <tr class="p-3 text-primary">
                                    <td class="p-3" colspan="2">1 year Total</td>
                                    <td class="p-3"> ₹49,000</td>
                                </tr>
                            </tbody>

                        </table>
                    </div>
                </div>
            </div>
        </div>

    </div>


    <!-- Ac room -->


    <div class="container my-5">
        <div class="container mt-5">
            <h4 class="text-secondary border-bottom p-3">~ AC ROOM : This is fees according to ac room </h4>
            <p class="p-2 text-muted"><b>NOTE : Addmission and hostel deposit fees only conduct by addmission time</b>
            </p>
            <div class="row">
                <!-- Single Seater Room -->
                <div class="col-md-4 mb-4">
                    <div class="border">
                        <div class="d-flex flex-column justify-content-center align-item-center"
                            style="background: rgba(0,0,0,0.5) url(hostel-image/single.jpg); background-blend-mode: darken;">
                            <h4 class="p-3 text-center text-white">Single Ac Seater Room</h4>
                            <button class="btn btn-danger btn-sm"><a href="hostel management System Project/hostel-fee/index.php">Apply now</a></button>
                        </div>
                        <table>
                            <thead>
                                <tr class="p-3 bg-dark text-white">
                                    <th class="p-3">Sr no.</th>
                                    <th class="p-3">Particular/month</th>
                                    <th class="p-3">Fees structure</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">1</td>
                                    <td class="p-3">Hostel addmision fees(one time)</td>
                                    <td class="p-3"> ₹500</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">2</td>
                                    <td class="p-3">Room rent</td>
                                    <td class="p-3"> ₹1000</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">3</td>
                                    <td class="p-3">Water & Electricity bill</td>
                                    <td class="p-3"> ₹4000</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">4</td>
                                    <td class="p-3">Medical checkup</td>
                                    <td class="p-3"> ₹300</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">5</td>
                                    <td class="p-3">Maintainance charges</td>
                                    <td class="p-3"> ₹200</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">6</td>
                                    <td class="p-3">Mess fees</td>
                                    <td class="p-3"> ₹2500</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">7</td>
                                    <td class="p-3">Hostel deposit(one time-Non-refundable)</td>
                                    <td class="p-3"> ₹500</td>
                                </tr>
                                <tr class="p-3 text-primary">
                                    <td class="p-3" colspan="2">6 Month Total</td>
                                    <td class="p-3"> ₹49,000</td>
                                </tr>
                                <tr class="p-3 text-primary">
                                    <td class="p-3" colspan="2">1 year Total</td>
                                    <td class="p-3"> ₹97,000</td>
                                </tr>
                            </tbody>

                        </table>
                    </div>
                </div>

                <!-- Double Seater Room -->
                <div class="col-md-4 mb-4">
                    <div class="border">
                        <div class="d-flex flex-column justify-content-center align-item-center"
                            style="background: rgba(0,0,0,0.5) url(hostel-image/single.jpg); background-blend-mode: darken;">
                            <h4 class="p-3 text-center text-white">Two Seater Room</h4>
                            <button class="btn btn-warning btn-sm"><a href="hostel management System Project/hostel-fee/index.php">Apply now</a></button>
                        </div>
                        <table>
                            <thead>
                                <tr class="p-3 bg-dark text-white">
                                    <th class="p-3">Sr no.</th>
                                    <th class="p-3">Particular/month</th>
                                    <th class="p-3">Fees structure</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">1</td>
                                    <td class="p-3">Hostel addmision fees(one time)</td>
                                    <td class="p-3"> ₹500</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">2</td>
                                    <td class="p-3">Room rent</td>
                                    <td class="p-3"> ₹1000</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">3</td>
                                    <td class="p-3">Water & Electricity bill</td>
                                    <td class="p-3"> ₹3000</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">4</td>
                                    <td class="p-3">Medical checkup</td>
                                    <td class="p-3"> ₹300</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">5</td>
                                    <td class="p-3">Maintainance charges</td>
                                    <td class="p-3"> ₹200</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">6</td>
                                    <td class="p-3">Mess fees</td>
                                    <td class="p-3"> ₹2500</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">7</td>
                                    <td class="p-3">Hostel deposit(one time-Non-refundable)</td>
                                    <td class="p-3"> ₹500</td>
                                </tr>
                                <tr class="p-3 text-primary">
                                    <td class="p-3" colspan="2">6 Month Total</td>
                                    <td class="p-3"> ₹43,000</td>
                                </tr>
                                <tr class="p-3 text-primary">
                                    <td class="p-3" colspan="2">1 year Total</td>
                                    <td class="p-3"> ₹85,000</td>
                                </tr>
                            </tbody>

                        </table>
                    </div>
                </div>
                <!-- Four Seater Room -->
                <div class="col-md-4 mb-4">
                    <div class="border">
                        <div class="d-flex flex-column justify-content-center align-item-center"
                            style="background: rgba(0,0,0,0.5) url(hostel-image/single.jpg); background-blend-mode: darken;">
                            <h4 class="p-3 text-center text-white">Four seater Room</h4>
                            <button class="btn btn-secondary btn-sm"><a href="hostel management System Project/hostel-fee/index.php">Apply now</a></button>
                        </div>
                        <table>
                            <thead>
                                <tr class="p-3 bg-dark text-white">
                                    <th class="p-3">Sr no.</th>
                                    <th class="p-3">Particular/month</th>
                                    <th class="p-3">Fees structure</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">1</td>
                                    <td class="p-3">Hostel addmision fees(one time)</td>
                                    <td class="p-3"> ₹500</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">2</td>
                                    <td class="p-3">Room rent</td>
                                    <td class="p-3"> ₹1000</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">3</td>
                                    <td class="p-3">Water & Electricity bill</td>
                                    <td class="p-3"> ₹2000</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">4</td>
                                    <td class="p-3">Medical checkup</td>
                                    <td class="p-3"> ₹300</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">5</td>
                                    <td class="p-3">Maintainance charges</td>
                                    <td class="p-3"> ₹200</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">6</td>
                                    <td class="p-3">Mess fees</td>
                                    <td class="p-3"> ₹2500</td>
                                </tr>
                                <tr class="p-3 text-success border">
                                    <td class="p-3">7</td>
                                    <td class="p-3">Hostel deposit(one time-Non-refundable)</td>
                                    <td class="p-3"> ₹500</td>
                                </tr>
                                <tr class="p-3 text-primary">
                                    <td class="p-3" colspan="2">6 Month Total</td>
                                    <td class="p-3"> ₹37,000</td>
                                </tr>
                                <tr class="p-3 text-primary">
                                    <td class="p-3" colspan="2">1 year Total</td>
                                    <td class="p-3"> ₹73,000</td>
                                </tr>
                            </tbody>

                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>
    
</section>
    <div class="container mb-5">
        <a href="register.php" class="btn pb-2 btn-dark">Register & book hostel room</a> 

        <h4 class="mx-4 pt-2 text-black">NOTE :-</h4>
        <ul>
            <li>Students who have their desktop/laptop in hostel will have to pay Rs. 200/- as computer electric charge
                extra per semester.</li>
            <li>Students of M.Phil./ Ph.D./Research Fellow etc. living in hostel in vacation period will have to pay Rs.
                250/- as extra room rent per semester.</li>
        </ul>
    </div>


    <!-- footer -->
    <div class="container-fluid bg-black text-white">
        <footer class="p-2">
            <div class="d-flex justify-content-center mb-4">
                <ul class="list-unstyled d-flex w-50 justify-content-around my-3">
                    <li class="ms-3"><a class="link-body-emphasis" href="#""><i class=" fa fa-2x fa-facebook
                            text-white"></i></a></li>
                    <li class="ms-3"><a class="link-body-emphasis" href="#"><i
                                class="fa fa-2x fa-instagram text-white"></i></a></li>
                    <li class="ms-3"><a class="link-body-emphasis" href="#"><i
                                class="fa fa-2x fa-youtube text-white"></i></a></li>
                    <li class="ms-3"><a class="link-body-emphasis" href="#"><i
                                class="fa fa-2x fa-envelope text-white"></i></a></li>
                </ul>
            </div>
            <a href="#top"><button class="btn btn-sm btn-danger mb-4">back to top</button></a>
            <div class="row">
                <div class="col-6 col-md-2 mb-3">
                    <h5 class="border-bottom pb-2">Navbar links</h5>
                    <ul class="nav flex-column">
                        <li class="nav-item mb-2"><a href="index.php" class="nav-link p-0 text-warning">Home</a></li>
                        <li class="nav-item mb-2"><a href="about.php" class="nav-link p-0 text-warning">About</a></li>
                        <li class="nav-item mb-2"><a href="Fees structure.php" class="nav-link p-0 text-warning">Fees
                                structure</a></li>
                        <li class="nav-item mb-2"><a href="gallery.php" class="nav-link p-0 text-warning">gallery</a>
                        </li>
                        <li class="nav-item mb-2"><a href="mess.php" class="nav-link p-0 text-warning">mess</a></li>
                    </ul>
                </div>

                <div class="col-6 col-md-2 mb-3">
                    <h5 class="border-bottom pb-2">Important form</h5>
                    <ul class="nav flex-column">
                        <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-warning">Hostel accommodation</a>
                        </li>
                        <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-warning">Student Declaration</a>
                        </li>
                        <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-warning">Mess rebate </a></li>
                        <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-warning">Visitor registration
                                form</a></li>
                        <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-warning">Medical
                                Reimbursement</a></li>
                    </ul>
                </div>

                <div class="col-6 col-md-2 mb-3">
                    <h5 class="border-bottom pb-2">Address</h5>
                    <ul class="nav flex-column">
                        <li class="nav-item mb-2"><span class="nav-link p-0 text-warning">
                                Mukerian , PO-Mukerian <br> Distt - Hoshiarpur <br>
                                State - Punjab(144211)
                                <br>
                                <br> Email : HOstel12423@gmail.com
                                <br>Phone : +91 135453154</span></li>
                    </ul>
                </div>

                <div class="col-md-5 offset-md-1 mb-3">
                    <form>
                        <h5>Share your feedback here</h5>
                        <div class="d-flex flex-column w-100 gap-2">
                            <label for="newsletter1" class="visually-hidden">Email address</label>
                            <input id="newsletter1" type="text" class="form-control" placeholder="Email address"
                                fdprocessedid="aohms7">
                            <label for="newsletter2" class="visually-hidden">Enter your feedback</label>
                            <textarea name="text" id="" cols="30" rows="4" class="form-control"></textarea>
                            <button class="btn btn-danger w-25" type="button" fdprocessedid="6y69j">Subscribe</button>
                        </div>
                    </form>
                </div>
            </div>

            <div class="border-top">
                <p class="text-center p-2">Design with love by Jasvir singh | ©2024 |~|OSTEL, Inc.
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;All rights reserved.</p>
            </div>
        </footer>
    </div>

    <script src="https://kit.fontawesome.com/26504e4a1f.js" crossorigin="anonymous"> </script>
    <script src="bootstrap-5.3.2-dist/js/bootstrap.bundle.min.js"></script>
    <script src="style.js"></script>
    <script type="text/javascript">
       function printDiv() {
        var printContents = document.getElementById('print-content').innerHTML;
        w=window.open();
        w.document.write(printContents);
        w.print();
        w.close();
    }
    </script>
</body>

</html>