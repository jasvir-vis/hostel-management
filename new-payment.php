...
if(isset($_POST['pay'])){
  $alsopaid = $_POST['alsopaid'];  
  $paid = $_POST['paiding'];
  $newpaid = $paid + $alsopaid;
  $fee = $_POST['pending'];
  $pending = $fee-$paid;
  $update = date('m-d-Y');
  $mode = $_POST['mode'];

  $payment = "UPDATE `payment` set paid_fees='$newpaid',pending='$pending',`update`='$update',mode='$mode' where email = '{$_SESSION['mail']}'";
        ...
        <div class="bg-white p-4 p-md-5 rounded shadow-sm">
          ...
          <form action="" method="post">
            <div class="row gy-3 gy-md-4 overflow-hidden">
                ...
                <input type="text" class="form-control" name="fname" placeholder="" value="<?php echo $row['email'];?>" readonly>
                </div>

                <div class="col-12">
                <label for="email" class="form-label">Total fees</label>
                <input type="text" class="form-control" name="total" placeholder="" value="<?php echo $row['total_fees'];?>" readonly>
                </div>
                
                <input type="hidden" name="alsopaid" value="<?php echo $row['paid_fees']; ?>">
                <div class="col-12">
                <label for="email" class="form-label">Pending fees</label>
                <input type="text" class="form-control" name="pending" placeholder="" value="<?php echo $row['pending'];?>">
                </div>
                <?php
                }
                ?>
